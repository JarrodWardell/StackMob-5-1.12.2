...
public class Commands implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] strings) {
        ...
        if (strings.length == 0) {
            commandSender.sendMessage(Utilities.PREFIX + ChatColor.RED + "Commands: ");
            for (SubCommand subCommand : subCommands) {
                ...
                commandSender.sendMessage(ChatColor.DARK_GREEN + "/" + cmd + " " + subCommand.getCommand() + " " + args + ChatColor.GRAY + "- " + ChatColor.GREEN + subCommand.getDescription());
            }
            commandSender.sendMessage(ChatColor.RED + "Key: () = Optional argument, [] = Mandatory argument.");
            return false;
        }
        for (SubCommand subCommand : subCommands) {
